...
The 'api_call' function should handle API calls to the Gaia API and return the response. My reference example function signature looks as follows:

```python
def api_call(base_url, endpoint, headers, payload=None, ssl_verify=False):
```

The 'logout' function should gracefully log your session out of the Gaia API.
